...
export const metadata: Metadata = {
  title: 'クラウドID管理「Keyspider」導入によるアカウント管理の自動化 - VIETISジャパン',
  description: '入退社・異動時のID管理をKeyspiderで完全自動化。手作業ゼロを実現した業務改善DXの実績詳細。',
};
...
.case-meta{display:flex;gap:24px;flex-wrap:wrap;font-size:13px;color:var(--subtle)}
.case-meta-item{display:flex;align-items:center;gap:6px}
.case-meta-item strong{color:var(--ink);font-weight:700}
.case-title-block{margin-bottom:clamp(40px,5vw,64px)}
.case-title-block .case-tags{margin-bottom:16px}
.case-title-block .page-hero-h1{margin-bottom:20px}
.page-hero-inner{padding:20px 0 16px}

/* HERO IMAGE */
.case-hero-img{width:100%;aspect-ratio:16/9;overflow:hidden;background:var(--bg-mid);border-radius:var(--r);margin-bottom:clamp(32px,4vw,48px)}
.case-hero-img img{width:100%;height:100%;object-fit:cover}

/* LAYOUT */
.case-layout{padding:clamp(24px,3vw,36px) 0 clamp(64px,8vw,96px)}
.case-layout .wrap{display:grid;grid-template-columns:1fr 300px;gap:clamp(48px,6vw,80px);align-items:start}
.case-body{}
.case-sidebar{position:sticky;top:96px}

/* CONTENT */
.case-section{margin-bottom:clamp(40px,5vw,60px)}
...
.case-h3{font-size:16px;font-weight:700;color:var(--ink);margin:28px 0 10px;line-height:1.5}
.case-p{font-size:16px;color:var(--ink);line-height:2.0;margin-bottom:20px}
.case-p:last-child{margin-bottom:0}
.case-ul{display:flex;flex-direction:column;gap:8px;margin-bottom:20px}
.case-ul li{font-size:16px;color:var(--ink);line-height:1.8;padding-left:16px;position:relative}
.case-ul li::before{content:'';position:absolute;left:0;top:0.65em;width:5px;height:5px;border-radius:50%;background:var(--blue)}

/* CHALLENGE / SOLUTION / RESULT BLOCKS */
.cs-grid{display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-bottom:clamp(32px,4vw,48px)}
.cs-card{background:var(--bg-off);border-radius:var(--r);padding:24px}
.cs-card-label{font-size:10px;font-weight:700;letter-spacing:0.1em;color:var(--subtle);margin-bottom:10px;font-family:var(--en)}
.cs-card-text{font-size:15px;color:var(--ink);line-height:1.8}

/* RESULT NUMBERS */
.result-nums{display:grid;grid-template-columns:repeat(3,1fr);gap:1px;background:var(--border);border:1px solid var(--border);border-radius:var(--r);overflow:hidden;margin-bottom:clamp(32px,4vw,48px)}
.result-num-item{background:var(--bg);padding:clamp(20px,3vw,28px) clamp(16px,2vw,24px)}
.result-num-value{font-family:var(--en);font-size:clamp(1.8rem,3vw,2.8rem);font-weight:700;letter-spacing:-0.03em;color:var(--blue);line-height:1;margin-bottom:8px}
.result-num-label{font-size:12px;color:var(--muted);line-height:1.55}

/* PROCESS */
.process-list{display:flex;flex-direction:column;gap:10px;counter-reset:step}
.process-item{display:grid;grid-template-columns:32px 1fr;gap:8px;padding:14px 20px;background:var(--bg-off);border-radius:var(--r)}
.process-num{font-family:var(--en);font-size:11px;font-weight:700;color:var(--subtle);padding-top:2px}
.process-title{font-size:15px;font-weight:700;color:var(--ink);margin-bottom:2px}
.process-desc{font-size:14px;color:var(--muted);line-height:1.75}

/* SIDEBAR */
.sidebar-card{background:var(--bg-off);border-radius:var(--r);padding:24px;margin-bottom:20px}
.sidebar-card-title{font-size:11px;font-weight:700;letter-spacing:0.08em;color:var(--subtle);margin-bottom:16px;font-family:var(--en)}
.sidebar-row{display:flex;flex-direction:column;gap:12px}
.sidebar-item{display:flex;flex-direction:column;gap:3px}
.sidebar-item-label{font-size:11px;color:var(--subtle)}
.sidebar-item-value{font-size:14px;font-weight:400;color:var(--ink);line-height:1.5}
.sidebar-cta{background:var(--blue);border-radius:var(--r);padding:24px;text-align:center}
.sidebar-cta-text{font-size:13px;color:rgba(255,255,255,0.7);line-height:1.7;margin-bottom:16px}
.sidebar-cta .btn{width:100%;justify-content:center;background:#fff;color:var(--ink-dark);border-color:#fff;font-size:13px;padding:11px 20px}
.sidebar-cta .btn:hover{background:#f0f0f0}

/* RELATED */
.related{padding:clamp(48px,6vw,72px) 0;background:var(--bg-page);border-top:1px solid var(--border)}
.related-head{font-size:clamp(18px,2vw,24px);font-weight:700;letter-spacing:-0.02em;color:var(--ink);margin-bottom:32px}
.related-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:clamp(24px,3vw,40px)}
.related-card{background:var(--bg);border-radius:var(--r);overflow:hidden;display:flex;flex-direction:column;transition:box-shadow 0.2s var(--ease),transform 0.2s var(--ease)}
@media(hover:hover) and (pointer:fine){
  .related-card:hover{box-shadow:0 4px 20px rgba(0,0,0,0.07);transform:translateY(-2px);outline:2px solid var(--blue)}
}
.related-thumb{aspect-ratio:16/10;overflow:hidden;background:var(--bg-mid)}
.related-thumb img{width:100%;height:100%;object-fit:cover;transition:transform 400ms var(--ease)}
.related-card:hover .related-thumb img{transform:scale(1.04)}
.related-body{padding:clamp(16px,2vw,24px)}
.related-tags{display:flex;gap:5px;flex-wrap:wrap;margin-bottom:10px}
.related-title{font-size:clamp(14px,1.5vw,18px);font-weight:700;color:var(--ink);line-height:1.55;transition:color 0.15s}
...
@media(max-width:1024px){
  .case-layout .wrap{grid-template-columns:1fr}
...
@media(max-width:768px){
  ...
  .result-nums{grid-template-columns:1fr 1fr}
  ...
  <div className="wrap">
    <div className="page-hero-inner">
      <nav className="breadcrumb reveal">
        ...
        <span aria-current="page">クラウドID管理「Keyspider」導入</span>
      </nav>
    </div>
  </div>
</div>
  ...
  <div className="wrap">
    ...
    <div className="case-body">
      ...
      <div className="case-hero-img reveal">
        <img src="https://picsum.photos/seed/vietis-case-keyspider-detail/1200/675" alt="クラウドID管理システム導入" loading="eager" />
      </div>
      ...
      <div className="case-title-block reveal">
        ...
        <h1 className="page-hero-h1">クラウドID管理「Keyspider」導入によるアカウント管理の自動化</h1>
      </div>

      
      <div className="case-section reveal">
        <h2 className="case-h2">プロジェクト概要</h2>
        <p className="case-p">従業員500名以上を抱えるIT企業において、入退社・組織変更のたびに発生するID管理業務が担当者の大きな負担になっていた。Active Directory、Google Workspace、Slack、各種SaaSツールなど、複数のシステムへの手動登録・削除が常態化し、対応漏れによるセキュリティリスクも課題となっていた。</p>
        <p className="case-p">VIETISは自社プロダクト「Keyspider」を中心に、既存のHRISとの連携設計から構築・テスト・運用移行まで一貫して支援。入退社・異動イベントに連動した自動プロビジョニング基盤を構築した。</p>
      </div>
      ...
      <div className="case-section reveal">
        ...
        <div className="cs-grid">
          <div className="cs-card">
            ...
            <p className="cs-card-text">入退社・異動のたびに担当者が手動で10以上のシステムを個別操作。月に数十件の申請が重なる繁忙期には対応が翌日以降にずれ込むことも。退職者アカウントの削除漏れによるセキュリティリスクも経営課題として認識されていた。</p>
          </div>
          <div className="cs-card">
            <p className="cs-card-label">SOLUTION</p>
            <p className="cs-card-text">HRISの人事イベントをトリガーに、Keyspiderが各SaaSへのアカウント操作を自動実行。承認フローも組み込み、管理者は内容確認だけで処理が完結する仕組みを構築。ログ記録によりセキュリティ監査対応も強化した。</p>
          </div>
        </div>
      </div>
      ...
      <div className="case-section reveal">
        ...
        <ul className="case-ul">
          <li>入退社・異動時のアカウント操作を全自動化。担当者の手作業をゼロに。</li>
          <li>退職者アカウントの削除漏れが解消され、セキュリティ監査での指摘事項が大幅減少。</li>
          <li>管理者の工数が月20時間以上削減され、本来業務に集中できる環境を実現。</li>
          <li>すべての操作ログが自動記録され、内部監査・コンプライアンス対応が容易になった。</li>
        </ul>
      </div>
      ...
      <div className="case-section reveal">
        ...
        <div className="process-list">
          <div className="process-item">
            ...
            <div>
              <div className="process-title">現状ヒアリングと連携先の棚卸し</div>
              <div className="process-desc">既存の人事フローと各SaaSの利用状況を整理。自動化対象となるシステムと操作ルールを定義した。</div>
            </div>
          </div>
          <div className="process-item">
            <div className="process-num">02</div>
            <div>
              <div className="process-title">Keyspider連携設計・構築</div>
              <div className="process-desc">HRISとKeyspiderの連携APIを設計。各SaaSへのプロビジョニングルールと承認フローを実装した。</div>
            </div>
          </div>
          <div className="process-item">
            <div className="process-num">03</div>
            <div>
              <div className="process-title">テスト・並行運用</div>
              <div className="process-desc">本番と同等の環境でシナリオテストを実施。問題がないことを確認したうえで段階的に移行した。</div>
            </div>
          </div>
          <div className="process-item">
            <div className="process-num">04</div>
            <div>
              <div className="process-title">本番移行・運用定着支援</div>
              <div className="process-desc">移行後も運用状況をモニタリング。組織変更に伴う設定追加などを継続的にサポートしている。</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="sidebar-card">
        ...
        <div className="sidebar-row">
          <div className="sidebar-item">
            <span className="sidebar-item-label">業種</span>
            <span className="sidebar-item-value">IT・サービス業</span>
          </div>
          <div className="sidebar-item">
            <span className="sidebar-item-label">従業員規模</span>
            <span className="sidebar-item-value">500名以上</span>
          </div>
          <div className="sidebar-item">
            <span className="sidebar-item-label">開発期間</span>
            <span className="sidebar-item-value">約4ヶ月</span>
          </div>
          <div className="sidebar-item">
            <span className="sidebar-item-label">チーム規模</span>
            <span className="sidebar-item-value">5名（PM・エンジニア・QA）</span>
          </div>
          <div className="sidebar-item">
            <span className="sidebar-item-label">技術スタック</span>
            <span className="sidebar-item-value">Keyspider / REST API / Active Directory / Google Workspace / Slack</span>
          </div>
          <div className="sidebar-item">
            <span className="sidebar-item-label">サービス種別</span>
            <span className="sidebar-item-value">Webシステム開発</span>
          </div>
        </div>
      </div>
  ...
  <div className="wrap">
    ...
    <div className="related-grid">
      <article className="related-card reveal">
        <a href="/cases/example-03/" className="related-thumb">
          <img src="https://picsum.photos/seed/vietis-case-legacy/720/450" alt="コア業務システム刷新" loading="lazy" />
        </a>
        <div className="related-body">
          ...
          <h3 className="related-title"><a href="/cases/example-03/">老朽化した社内コア業務システムの全面刷新・Webシステム化</a></h3>
        </div>
      </article>
      <article className="related-card reveal d1">
        <a href="/cases/example-10/" className="related-thumb">
          <img src="https://picsum.photos/seed/vietis-case-hreval/720/450" alt="人事評価システム" loading="lazy" />
        </a>
        <div className="related-body">
          <div className="related-tags">
            <span className="tag tag-ind">業務改善・DX</span>
          ...
          <h3 className="related-title"><a href="/cases/example-10/">プロジェクト稼働データと自動連携する人事評価システムの構築</a></h3>
        </div>
      </article>
      <article className="related-card reveal d2">
        <a href="/cases/example-11/" className="related-thumb">
          <img src="https://picsum.photos/seed/vietis-case-docmgmt/720/450" alt="帳票作成・管理システム" loading="lazy" />
        </a>
        <div className="related-body">
          <div className="related-tags">
            <span className="tag tag-ind">業務改善・DX</span>
          ...
          <h3 className="related-title"><a href="/cases/example-11/">複雑な承認プロセスに対応した帳票作成・管理システムの開発</a></h3>
        </div>
      </article>
    </div>
  </div>
